...
static void ui_nl_set_state(int n)
{
	...
	pr_debug("n is %x. Action flags being changed from %lx to %lx.", n, toi_bkd.toi_action, new_action);
 ...
 *
 * Tell userui that atomic restore just occurred, so that it can do things like
...
static int userui_user_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	if (type == NETLINK_MSG_NOFREEZE_ME && ui_helper_data.pid != -1) {
		pr_warn("Got NOFREEZE_ME request when ui_helper_data.pid is %d.\n", ui_helper_data.pid);
		return -EBUSY;
	}
...
static void userui_prepare_console(void)
{
	...
	if (!userui_ops.enabled) {
		pr_warn("TuxOnIce: Userui disabled.\n");
		return;
	}
	...
	if (*ui_helper_data.program)
		...
		pr_warn("TuxOnIce: Userui program not configured.\n");
}
